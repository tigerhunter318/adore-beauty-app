...
import {NavigationState, PartialState, PathConfig, PathConfigMap, Route} from "@react-navigation/native";
...
//
    ...
    if (route.state) {
      ...
      let query = queryString.stringify(focusedParams);

      if(focusedParams?.screen && focusedParams?.params){
        query = queryString.stringify(focusedParams.params, { sort: false });
        path += `/${focusedParams.screen}?${query}`;
      }else if(query){
        path += `?${query}`;
      }
    }
